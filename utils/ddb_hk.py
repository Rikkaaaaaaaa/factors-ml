...
DDB_config = {"server": "10.50.129.65",
              "port": 18921,
              "userName": "g_eq_algo",
...
def read_ddb_factor_hk(data_base, table_name, test_month, tickers, trading_hours=None):
...
def read_ddb_factor_by_ticker_hk(data_base, table_name, test_month, tickers, trading_hours=None,
...
def read_ddb_factor_low_price_hk(data_base, table_name, test_month, tickers, trading_hours=None):
    test_month = str(test_month)
    test_month = test_month[0:4] + '.' + test_month[4:] + 'M'
    ...
    scripts = "factorTable = loadTable(\"{}\", \"{}\")".format(data_base, table_name)
    ddb_reader.ddb_session.run(scripts, priority=9)
    scripts = "retTable = select * from factorTable where month(time)={} and securityCode in {}".format(test_month,
                                                                                                        tickers)
    ddb_reader.ddb_session.run(scripts, priority=9)
    scripts = "select factorValue from retTable pivot by time, securityCode, factorName"
    factor = ddb_reader.ddb_session.run(scripts, priority=9)
    ...
    # filter by select_period dict
    if isinstance(trading_hours, dict):
        factor = factor[((factor["time"] >= trading_hours['am_start_time'] * 1000) & (
                    factor["time"] <= trading_hours['am_end_time'] * 1000))
                        | ((factor["time"] >= trading_hours['pm_start_time'] * 1000) & (
                    factor["time"] <= trading_hours['pm_end_time'] * 1000))
                        ]
    else:
...
def read_ddb_return_hk(data_base, table_name, test_month, tickers, trading_hours=None):
    test_month = str(test_month)
    test_month = test_month[0:4] + '.' + test_month[4:] + 'M'
    ddb_reader = DDB_connector(DDB_config)
    ...
    # filter by select_period dict
    if isinstance(trading_hours, dict):
        ret = ret[((ret["time"] >= trading_hours['am_start_time'] * 1000) & (
                    ret["time"] <= trading_hours['am_end_time'] * 1000))
                  | ((ret["time"] >= trading_hours['pm_start_time'] * 1000) & (
                    ret["time"] <= trading_hours['pm_end_time'] * 1000))
                  ]
    else:
        # else time between [94000, 145700]
...
def get_limit_flag_hk(month, ticker_list):
