...
from dataset import build_factor_name_hk
from dataset.sql_ops import load_ticker_by_indus_hk, load_labels_hk, align_factor_ticker_hk, load_factor_by_table_hk, \
    check_rebalanced
...
class FactorHKDataset():
    ...
    def __init__(self, opt, test_month, indus_type, logger_name=None, *args, **kwargs):
        ...
        # build factor names
        self.training_factor_name = build_factor_name_hk(
            self.opt['dataset']['training_factor_name'])  # list(set(_train_data.columns) - set(del_column))
        self.std_factor_name = build_factor_name_hk(self.opt['dataset']['std_factor_name'])
        self.clip_factor_name = build_factor_name_hk(self.opt['dataset']['clip_factor_name'])
        self.log_factor_name = build_factor_name_hk(self.opt['dataset']['log_factor_name'])
        ...
        self.logger.info(
            f"[{self.logger_name}] Loading indus by [{self.indus_class}] from table [static_data_industry_{self.pool_name}_history] + [{self.opt['dataset']['indus_table_suffix']}]]")
        if isinstance(self.trading_hours, dict):
            if self.trading_hours['am_start_time'] <= self.trading_hours['am_end_time']:
                self.logger.info(
                    f"[{self.logger_name}] Trading hours: AM [{self.trading_hours['am_start_time']}, {self.trading_hours['am_end_time']}]")
            if self.trading_hours['pm_start_time'] <= self.trading_hours['pm_end_time']:
                self.logger.info(
                    f"[{self.logger_name}] Trading hours: PM [{self.trading_hours['pm_start_time']}, {self.trading_hours['pm_end_time']}]")
    ...
    def load_data(self, cache=False, data=None):
        ...
        try:
            ...
            if self.debug_mode:
                ...
                self.logger.info(
                    f"[{self.logger_name}] Enable DEBUG: ticker num has been truncted to {len(self.tickers)}")
            ...
            # count data num
            self.logger.info(
                f"[{self.logger_name}] Training data num is {len(train_data)} and test data num is {len(test_data)}")
    ...
    def load_ticker_list(self):
        ...
        try:
            ...
            cur_ticker = load_ticker_by_indus_hk(self.opt, self.pool_name, self.indus_type, self.test_month)
            ticker_list.extend(cur_ticker)
            self.logger.info(
                f"[{self.logger_name}] Suscessfully load ticker list from mysql with cost={time_str(load_timer.item())}")
            ...
            ticker_list = align_factor_ticker_hk(self.factor_table, all_ticker, self.pool_name, check_ticker_month,
                                              self.test_month, self.rebalancing_tables, self.training_month_num,
                                              self.io_backend, self.logger_name)
            self.logger.info(
                f"[{self.logger_name}] Suscessfully align ticker list with cost={time_str(load_timer.item())}")
        ...
        try:
            ...
            for database in database_name:
                for table in factor_table_name[database]:
                    ...
                    factor = load_factor_by_table_hk(database, table, self.tickers, self.pool_name, month, self.test_month,
            ...
            # load labels
            labels = load_labels_hk(self.opt, self.tickers, month)
            ...
            if len(missing_tickers) > 0:
                ...
                self.logger.warning(
                    f"[{self.logger_name}] There are missing tickers in {month} Return: {list2str(missing_tickers)}")
    ...
    def filter_by_label(self, train_data):
        ...
        if self.task_type == 'classification':
            ...
            self.logger.info(
                f"[{self.logger_name}] Select {len(train_data) / total_train_num * 100:.2f}% train data with return={self.alpha} and class num is {self.class_num}.")
    ...
    def del_train_value(self, train_data):
        ...
        if self.opt['dataset'].get('drop_null_in_training'):
            self.logger.info(
                f"[{self.logger_name}] Delete {(len(train_data) - len(data)) / len(train_data) * 100:.2f}% null samples in train data.")
            return data
        else:
            pass
            # keep nan by default
            ...
            self.logger.info(
                f"[{self.logger_name}] Keep nan in training data. There are {(len(train_data) - len(data)) / len(train_data) * 100:.2f}% null samples in train data.")
        return train_data
            ...
            for ticker in self.tickers:
                ...
                if len(self.clip_factor_name) > 0:
                    ...
                    if self.mode == 'eval':
                        ...
                        factor_min = \
                        ticker_clip_params.set_index('factor_name').loc[self.clip_factor_name].reset_index()[
                            'min'].values
                        factor_max = \
                        ticker_clip_params.set_index('factor_name').loc[self.clip_factor_name].reset_index()[
                            'max'].values
                ...
                if len(self.std_factor_name) > 0:
                    ...
                    if self.mode == 'eval':
                        ...
                        factor_std = ticker_std_params.set_index('factor_name').loc[self.std_factor_name].reset_index()[
                            'std'].values
                        factor_mean = \
                        ticker_std_params.set_index('factor_name').loc[self.std_factor_name].reset_index()[
                            'mean'].values
                    else:
...
if __name__ == '__main__':
    ...
    indus_type = 1
    dataset = FactorHKDataset(opt, 202605, indus_type)
    dataset.load_data()
