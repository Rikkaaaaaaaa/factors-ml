...
class FactorDataset2():
    ...
    def __init__(self, opt, test_month, indus_type, cache=None, *args, **kwargs):
        ...
        self.database = 'factor_test'
        #self.factor_names = batch2_factor_names  # list(set(_train_data.columns) - set(del_column))
        #self.std_factor_names = base_factor
        #self.clip_factor_names = base_factor + ofl_factor
        # self.factor_names = base_factor
        # self.std_factor_names = base_factor
        # self.clip_factor_names = base_factor
        self.tickers = []
    ...
    def load_ticker_list(self):
        ...
        for pool in self.pool_name:
            ...
            for month in self.training_month + [self.test_month]:
                factor_ticker = cx_read_sql('select distinct ticker from all_factor_{}'.format(month), database=self.database)
    ...
    def load_data_from_sql(self, month):
        try:
            ...
            if is_rebalanced and month % 100 in [4, 5, 6, 10, 11, 12]:
                ...
                factor = cx_read_sql('select * from all_factor_{} where ticker in {}'.format(month, self.tickers), database=self.database)
                labels = cx_read_sql('select ticker, date, time, ret_{}  from ret_{} where ticker in {}'.format(self.opt['dataset']['ret_name'], month, self.tickers))


            # log factor
            log_factors = log_factor_list
            ...
            #self.logger.info("SQL data missing ticker", list(missing_tickers))
    ...
    def transform(self, train_data, test_data=None):
        ...
        for ticker in self.tickers:
            ...
            train_x = _train_data[self.std_factor_names]
            test_x = _test_data[self.std_factor_names]
            # data std
            ...
            self.train_data.loc[_train_data.index, self.std_factor_names] = train_x.values
            self.test_data.loc[_test_data.index, self.std_factor_names] = test_x.values

            # data clip
            train_x = _train_data[self.clip_factor_names]
            test_x = _test_data[self.clip_factor_names]
            ...
            # restore data at original position
            self.train_data.loc[_train_data.index, self.clip_factor_names] = train_x
            self.test_data.loc[_test_data.index, self.clip_factor_names] = test_x
            # save transform params
            # transform_param['factor_name'] = self.factor_names
    ...
    def transform_runtime(self, train_data):
        del_column = ['time', 'ticker', 'date', 'class_label', 'ret']          # del columns in data
        factor_names = batch2_factor_list
        self.train_data = train_data.reset_index()
        self.transform_params = []
        ...
        for ticker in self.tickers:
            ...
            # save transform params
            transform_param['factor_name'] = batch2_factor_list
            transform_param['mean'] = factor_mean
            transform_param['std'] = factor_std
            transform_param['min'] = factor_min
            transform_param['max'] = factor_max
            ...
            self.transform_params.append(transform_param)
            # restore data at original position
            self.train_data.loc[_train_data.index, batch2_factor_list] = train_x
    ...
    def rebalance_training_data(self):
        ...
        if self.class_num == 2 and self.opt['dataset']['balance'] == 'reverse':
            ...
            train_data_copy[self.factor_names] = -1 * train_data_copy[self.factor_names]
...
if __name__ == '__main__':
    ...
    indus_type = 1
    dataset = FactorDataset2(opt, 202307, indus_type)
    dataset.load_data()
